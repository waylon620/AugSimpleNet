...
_DATASETS = {
    "mvtec": ["datasets.mvtec", "MVTecDataset", "MVTecDataset_aug"],
}
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        for i, SimpleNet in enumerate(simplenet_list):
            ...
            if not test:
                i_auroc, p_auroc, pro_auroc = SimpleNet.train(dataloaders["training_view"], dataloaders["testing"])
            else:
    ...
    def get_dataloaders(seed):
        ...
        for subdataset in subdatasets:
            train_dataset = dataset_library.__dict__[dataset_info[1]](
                ...
                seed=seed,
                rotate_degrees=0, # RandomAffine
                translate=0, # RandomAffine
                brightness_factor=0,
                contrast_factor=0,
                saturation_factor=0,
                gray_p=0,
                h_flip_p=0,
                v_flip_p=0,
                scale=0, # RandomAffine
                std=0,
                augment=augment,
            )
            '''train_dataset_view1 = dataset_library.__dict__[dataset_info[1]](
                data_path,
                classname=subdataset,
                resize=resize,
                train_val_split=train_val_split,
                ...
                split=dataset_library.DatasetSplit.TRAIN,
                seed=seed,
                rotate_degrees=rotate_degrees, # RandomAffine
                translate=translate, # RandomAffine
                ...
                h_flip_p=0,
                v_flip_p=0,
                scale=scale, # RandomAffine
                std = 0.02,
                augment=augment,
            )
            train_dataset_view2 = dataset_library.__dict__[dataset_info[1]](
                data_path,
                classname=subdataset,
                resize=resize,
                train_val_split=train_val_split,
            ...
            train_dataset_view = dataset_library.__dict__[dataset_info[2]](
                data_path,
                classname=subdataset,
                resize=resize,
                train_val_split=train_val_split,
                ...
                split=dataset_library.DatasetSplit.TRAIN,
                seed=seed,
                rotate_degrees=(rotate_degrees, rotate_degrees), # RandomAffine
                translate=(translate, translate), # RandomAffine
                ...
                h_flip_p=(0, 0),
                v_flip_p=(0, 0),
                scale=(scale, scale), # RandomAffine
                std = (0.05, 0.05),
                augment=augment,
            )
            ...
            train_view_dataloader = torch.utils.data.DataLoader(
                train_dataset_view,
                batch_size=batch_size,
                shuffle=True,
                num_workers=num_workers,
            ...
            dataloader_dict = {
                ...
                "training_view": train_view_dataloader,
                # "training_merged": train_dataloader_merged,
...
if __name__ == "__main__":
    wandb.init(project="AD_final",
                name="Augmented_Simplenet",
                config={
                "dataset": "all",
                "meta_epochs": 40,
                "batch_size": 16,
                "gan_epochs": 4,
                "run_name": "run_meta_epochs_40",
                })
