...
class SimpleNet(torch.nn.Module):
    ...
    def load_state_dict(self, state_dict):
        """ Custom function to load the checkpoint"""
    ...
    def _train_adaptor(self, training_data, meta_epochs):
        #LOGGER.info(f"Training adaptor...")
        self.pre_projection.train()
        for data in training_data:
            self.proj_opt.zero_grad()
            img = data["image"].to(torch.float).to(self.device)
            feats = self.pre_projection(self._embed(img, evaluation=False)[0])
            
            img_view1 = data["image_view1"].to(torch.float).to(self.device)
            feats_view1 = self.pre_projection(self._embed(img_view1, evaluation=False)[0])
            img_view2 = data["image_view2"].to(torch.float).to(self.device)
            feats_view2 = self.pre_projection(self._embed(img_view2, evaluation=False)[0])
            
            criterion = torch.nn.MSELoss()
            # loss = 0
            loss = criterion(feats_view1, feats_view2)
            loss += criterion(feats, feats_view1)
            loss += criterion(feats, feats_view2)
            loss *= 1e2
            loss.backward()
            self.proj_opt.step()
            wandb.log({"Adaptor:  loss": loss.item(), "epoch": meta_epochs})

    def _train_discriminator(self, input_data, meta_epochs):
        ...
        i_iter = 0
        LOGGER.info(f"Training adaptor and discriminator...")
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                all_p_interp = []
                embeddings_list = []
                self._train_adaptor(input_data, meta_epochs)
                for data_item in input_data:
                    ...
                    wandb.log({"Discriminator:  loss": loss.item(), "meta_epochs": meta_epochs,"gan_epochs": i_epoch })
    ...
    def forward(self, images):
        ...
        with torch.no_grad():
            ...
            features = features.reshape(batchsize, scales[0], scales[1], -1)
            masks, features = self.anomaly_segmentor.convert_to_segmentation(patch_scores, features)
        return {"pred_score": torch.from_numpy(image_scores), "anomaly_map": torch.tensor(np.array(masks))}
        #return list(image_scores), list(masks), list(features)
